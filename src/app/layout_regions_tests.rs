...
fn cell_outside_every_surface_is_none() {
    ...
    assert_eq!(r.target_at(5, 11, PROMPT_W, 0), None);
    // x past the pane width.
    assert_eq!(r.target_at(100, 3, PROMPT_W, 0), None);
}
...
fn click_in_query_bar_maps_to_text_col_past_the_prompt() {
    ...
    assert_eq!(
        r.target_at(5, 10, PROMPT_W, 0),
        Some(MouseTarget::QueryBar { col: 3 })
    );
    // A click on the prompt itself clamps to text col 0.
    assert_eq!(
        r.target_at(0, 10, PROMPT_W, 0),
        Some(MouseTarget::QueryBar { col: 0 })
    );
    assert_eq!(
        r.target_at(1, 10, PROMPT_W, 0),
        Some(MouseTarget::QueryBar { col: 0 })
    );
}
...
fn popup_inner_rows_count_from_one_below_the_top_border() {
    ...
    for (screen_row, want) in [(5u16, Some(0usize)), (6, Some(1)), (7, Some(2))] {
        assert_eq!(
            r.target_at(3, screen_row, PROMPT_W, 0),
            Some(MouseTarget::Popup {
...
fn empty_regions_resolve_to_none() {
    ...
    assert_eq!(r.target_at(0, 0, PROMPT_W, 0), None);
    assert_eq!(r.target_at(10, 10, PROMPT_W, 0), None);
}
...
fn zero_height_pane_has_no_body_row() {
    ...
    assert_eq!(r.target_at(5, 0, PROMPT_W, 0), None);
}
