...
/// Ctrl+K key event.
fn ctrl_k() -> KeyEvent {
    KeyEvent::new(Key::Char('k'), super::KeyMods::CTRL)
}
...
fn ctrl_c_quits_even_with_palette_open() {
    let (mut app, _rx) = loaded_app();
    app.on_key(ctrl_k(), 0);
    let quit = app.on_key(KeyEvent::new(Key::Char('c'), super::KeyMods::CTRL), 0);
...
fn ctrl_c_quits_even_with_facet_open() {
    ...
    app.on_key(KeyEvent::char('f'), 300);
    let _ = facet_request(&rx);
    let quit = app.on_key(KeyEvent::new(Key::Char('c'), super::KeyMods::CTRL), 400);
