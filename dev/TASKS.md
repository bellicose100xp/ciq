...
| P1.6 | `src/harness/` — `EngineHarness` (load fixture, fire SQL, assert `QueryOutcome`) | P1.4, P1.5 | TODO | At least one self-test runs with no TTY (`TERM` unset). Deterministic. |
| P1.7 | `AppHarness` minimal stub (drives `App` against `ratatui::TestBackend`; feeds synthetic `current_time_ms: u64`) | P1.6 | TODO | Renders an empty/loading frame to `TestBackend`; self-test asserts no real TTY needed. Fleshed out in P2. |
...
| ID | Task | Deps | Status | Exit criteria |
|---|---|---|---|---|
| P2.1 | `src/query/debouncer.rs` — port jiq's time-as-`u64`-parameter debouncer (150ms) verbatim | P1.1 | TODO | `should_execute_at(now_ms)` / `schedule_execution_at(now_ms)`; tests pass explicit `u64` at exact boundaries; no Clock trait. |
...
| P2.4 | `src/query/query_state.rs` — latest-`request_id` tracking + stale-discard; `is_stale(incoming, latest)` pure fn | P2.2 | TODO | `is_stale` unit-tested exhaustively; stale responses dropped before touching result state. |
...
| P2.9 | `src/query/preprocess.rs` — SQL normalize + §2.3 LIMIT-wrap (tokenize-only; suppress wrap if top-level LIMIT present) | P2.2 | TODO | Pure `String -> String`; table-driven tests incl. trailing `;`, existing `LIMIT`, reject multi-statement/DML. |
| P2.10 | `src/query/error_enhance.rs` — DuckDB error -> friendly message | P2.2 | TODO | Pure mapping, golden table tests. |
