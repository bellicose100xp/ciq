...
- **S6 — `ProcessedResult` added fields:** carries structured grid data as `grid: GridLayout` plus `rows` + `schema` (per §3.2), **not** a flat `column_widths` (§7.3's outlier). One authoritative field list; §7.3 corrected. The §3.2 note "corrects §7.2" is repointed to §7.3 (where `ProcessedResult` is actually discussed). **Grid type name (P2.6):** the produced layout struct is named **`GridFrame`** (matches the P2.6 exit criteria: `layout_grid -> GridFrame{header, body, col_x, total_width}`). **`GridLayout` is a type alias for `GridFrame`** (`src/grid/grid_layout.rs`); the canonical name is `GridFrame`, and the worker's `ProcessedResult.grid: GridLayout` field refers to the same type. The worker subsystem references `GridFrame` (or the `GridLayout` alias) interchangeably.

---

## Deferred to implementation (decide at CSV-ingest time, Phases 2/4 — NOT now)

These carried real correctness weight but did not gate Phase 1. **All four are now RESOLVED in P4.7** (see the Q3/Q7/Q12 + CsvOpts entries below). **Deferred correctness items CLOSED (P5.5, 2026-06-07):** Q3/Q7/Q12 are each decided (entries below) and fixture-tested against the real `duckdb 1.10503.1` engine. P4.7 landed the bulk; P5.5 verified the closure and added the one missing edge — Q3's emit-time-quoting *round-trip* (a reserved-word / special-char column queried end-to-end via `quote_ident`, `tests/fixtures/quoted_names.csv`) — and corrected two stale Q7/Q12 lines in the `src/ingest.rs` module doc that still carried the pre-resolution (wrong) wording. The original deferral notes:
...
**Implementation:** one shared escaper — **`src/sql_ident.rs`** (`quote_ident`, `quote_ident_if_needed`, `single_quote_literal`, `is_reserved_keyword`, and — added by the P4 adversarial-review fix below — the shared `is_bare_literal` / `render_typed_value` bare-vs-quote predicate) — promoted out of `autocomplete/value_source.rs` to a neutral top-level home so `ingest` / `palette` / `facets` share it without importing `autocomplete` (the §0/D2 anti-coupling rule). The reserved-word check delegates to `sql_lexer::is_reserved_keyword` (the single keyword table), so "what the lexer re-reads as a keyword" and "what gets quoted on emit" can never drift. `value_source::quote_ident` and `insertion.rs` now re-use it (the old duplicated `quote_ident_if_needed` / `needs_quoting` / `single_quote` were deleted — DRY). `quote_ident_if_needed` no longer exempts a bare `*`: a real column literally named `*` is quoted (`"*"`), so the palette projection can't silently widen to all columns; the all-columns wildcard is a directly-built `*` literal (empty-selection emit + the autocomplete SELECT-list wildcard suggestion), never routed through the identifier quoter.
...
**Emit-time-quoting round-trip (P5.5 gap-fill, `tests/fixtures/quoted_names.csv`, headers `order,Total ($),select`):** a column whose raw name needs quoting — a reserved word (`order`, `select`) or a special-char name (`Total ($)`) — survives verbatim into the schema and is *queryable end-to-end* through the real engine via the shared `quote_ident`. Pinned by `ingest_semantics_tests::q3_quoting_required_column_names_round_trip_through_a_real_query`, which also asserts the bare (unquoted) `SELECT order FROM t` is a syntax error — so the quoted round-trip isn't vacuously true. This closes Q3's other half: the earlier dedup/survival fixtures proved names *exist*; this proves the emit policy actually *runs* them.
...
## Phase 4 adversarial-review fixes (2026-06-07)

Confirmed defects from the Phase 4 adversarial review, fixed with regression tests. Each item names the root cause and the guard that now pins it.

- **Engine value rendering (critical→fixed).** `value_ref_to_cell`'s catch-all was `format!("{other:?}")` on the `ValueRef` enum, which only derives `Debug` — so every DATE/TIMESTAMP/TIME/DECIMAL/BLOB/INTERVAL cell entered the table as variant-name garbage (`Date32(19372)`, `Decimal(1250.50)`), corrupting the grid and all four `--output` writers from the documented default `SELECT *`. New pure module **`src/engine/value_render.rs`** renders each to DuckDB's canonical text (civil-from-days date math, microsecond-trimmed timestamp/time, scale-preserving decimal, hex-escaped blob); on the hard floor. Guards: `value_render_tests`, `duckdb_engine_tests::{temporal_and_decimal,timestamp_and_time}_cells_render_faithfully`, `tests/output_cli.rs::output_{csv,json}_renders_date*`.

- **Sniffer header assertion (high→fixed).** `detect_header` returned a hard `Some(false)` for an all-text body with no numeric row to contrast — an *assertion* in the merge pipeline that overrode DuckDB's correct header detection and swallowed the header of a common all-text CSV (columns became `column0/column1`). Now returns `Option<bool>`: `None` (defer to DuckDB) for the undetermined all-text case, `Some(false)` only when row 0 itself is numeric. Guards: the rewritten `sniff_tests` header cases + `ingest_semantics_tests::q3_all_text_header_survives_default_load` (end-to-end via the real engine).

- **DRY: one bare-vs-quote predicate (low→fixed).** `is_bare_literal` was duplicated with *divergent* semantics between `palette/query_emit` and `autocomplete/insertion` (the latter bared any non-`inf`/`NaN` string), and a doc comment falsely claimed they were "kept consistent." Hoisted one `is_bare_literal` + `render_typed_value` into `sql_ident.rs` (the safer query_emit semantics); both emitters call it. Also closed the operator-blind `LIKE` edge: a `LIKE`/`NOT LIKE` value now always single-quotes (a pattern is a string regardless of column type).

- **DRY: one fuzzy-filter primitive (low→fixed).** `is_subsequence` was copy-pasted in `palette_state` and `autocomplete/candidates`. Lifted into the neutral leaf **`src/text_match.rs`** (mirroring the `sql_ident` lift) both import; on the hard floor.

- **Facet all-NULL null count (low→fixed).** The histogram SQL carried distinct/null counts as per-row correlated sub-selects, so an entirely-NULL text column (zero `GROUP BY` rows) lost its null count (reported `nulls=0`). Rebuilt as `stats LEFT JOIN bars ON true` so the counts always materialize (one sentinel `(NULL, NULL, distinct, null_count)` row when there are no bars); the parser skips the NULL-value sentinel. Guards: `facet_state_tests::all_null_histogram_reports_real_null_count_via_sentinel_row` + `facet_engine_tests::all_null_text_histogram_reports_real_null_count`.

- **Value-completion order determinism (low→fixed).** `build_distinct_sql` (and the engine's `distinct`) ordered only `n DESC`, leaving ties unspecified — so the suggestion list (and which tied values survive the LIMIT) could vary run to run. Added the `, 1 ASC` value tie-break, matching the facet histogram. Guard: `value_source_tests::distinct_sql_carries_value_tie_break_for_stable_order`.

- **`app_tests.rs` over the 1000-line limit (medium→fixed).** Split the palette (P4.4/P4.5) and instant-facet (P4.6) App-shell test blocks into `src/app/app_tests/{palette_tests,facet_tests}.rs`, wired via `#[path]` (explicit, because `app_tests.rs` is itself `#[path]`-loaded), sharing the App helpers via `super`.

---

## Phase 5 adversarial-review fixes (2026-06-07)

Confirmed defects from the Phase 5 adversarial review, fixed with regression tests. Each item names the root cause and the guard that now pins it. **869 lib + 8 integration tests** after the pass (was 843+8).

- **`[general]` row_limit / threads / memory_limit documented-but-dead (high→fixed).** The whole `[general]` section was parsed into `GeneralConfig` (its accessors on the hard floor, coverage green) but **never read at runtime**: the App hardcoded `VIEWPORT_ROW_LIMIT = 1000`, the engine hardcoded `SET threads = 4`, and `memory_limit` was never issued — while `configuration.md` / `live-sql-grid.md` documented all three as live (a green-coverage-over-inert-config gap). **Wired all three.** New plain-data `engine::EngineConfig { threads, memory_limit }` (the config-free projection of `[general]` the engine consumes); `DuckdbEngine::open_with(path, opts, &EngineConfig)` applies `SET threads` (folding in the bounded `DEFAULT_THREADS=4` when `None`) and `SET memory_limit='<s>'` when set — a malformed size string surfaces as `EngineError::Load` (the documented "malformed → clean load error" contract), never a panic. The App gained a `viewport_row_limit` field + `configure_general(row_limit)`; `prepare_interactive`, the truncation banner, and the palette emit (`emit_with_limit`) now use it. `event_loop` (interactive) and `main.rs` (`--output`) build the `EngineConfig` from `cfg.general()` and call `open_with`; the App's row cap is seeded via `configure_general(cfg.general().row_limit())`. Guards: `duckdb_engine_tests::{open_with_applies_configured_threads_pragma, open_with_applies_valid_memory_limit, open_with_malformed_memory_limit_yields_load_error}`, `polish_tests::{configured_row_limit_changes_the_dispatched_limit, configured_row_limit_drives_the_truncation_banner_cap, configure_general_clamps_zero_to_one}`.

- **In-memory history ring never capped to `max_entries` (low→fixed).** `HistoryState::add`/`load_entries` deduped + inserted but never truncated, so the in-session `Vec<String>` grew unbounded across a long session — contradicting the documented "cap bounds the in-session ring + on-disk file" (`history_config.rs:26`, `configuration.md:85`); only the on-disk file was bounded. **Gave the ring its cap:** `HistoryState` now carries a `max` field (manual `Default` = `DEFAULT_MAX_ENTRIES`, reused from the `[history]` config default), with `with_max`/`with_entries_max`/`set_max` constructors and a `trim_to_max` applied at the end of `add`/`load_entries`. `App::configure_history` threads the configured cap into the ring (seeded-from-disk and session-only both). Guards: `history_state_tests::{add_beyond_the_cap_drops_the_oldest_entry, with_max_clamps_zero_to_one, with_entries_max_caps_seeded_entries, set_max_shrinks_an_existing_ring, default_ring_carries_the_built_in_cap}`.

- **`[ai]`/`[general]`/`[theme]`/`[history]` accepted unknown keys; only `[csv]` denied them (low→fixed).** The config module doc promised "each known section validates its own keys", but only `CsvConfig` carried `deny_unknown_fields` — a typo in any other section (and, for `[ai]`, a stray `api_key = "sk-…"` mistaken for `api_key_env`) was silently dropped with no warning, leaving a real secret at rest in a plaintext config while the tool reported everything fine. **Added `deny_unknown_fields` to all four remaining section structs** (matching `[csv]`), so a typo'd / secret-adjacent key falls through to the existing "invalid config, using defaults" warning path. `[theme.overrides]` stays an open map (its arbitrary inner keys are still accepted). The top-level `Config` stays permissive (forward-compat for unknown `[export]`-style tables, pinned by `unknown_top_level_table_is_tolerated`). Guards: `config_tests::{unknown_general_key_falls_back_to_default_with_warning, unknown_theme_key_falls_back_to_default_with_warning, theme_overrides_map_still_accepts_arbitrary_inner_keys, unknown_history_key_falls_back_to_default_with_warning, raw_api_key_in_ai_section_is_rejected_with_warning}`.

- **AI replies wrapped in markdown code fences rejected as not-read-only (low→fixed).** `on_ai_result` did only `sql.trim()`, so a `` ```sql … ``` `` reply (a very common LLM habit, despite the prompt's best-effort no-fences rule) lexed with a leading `` ` ``/`sql` token and was rejected by preprocess as "read-only SELECT queries only" — a happy-path usability cliff. **Added pure `prompt::strip_code_fences`** (unwraps a single outer fence + surrounding whitespace) called in `on_ai_result` before `set_text`; the unwrapped SQL still crosses the **same** read-only guard (preprocess stays the boundary — a fenced DML reply is still rejected). Guards: `prompt_tests::strip_code_fences_*` (tagged / untagged / whitespace / unfenced / unterminated) + `ai_tests::fenced_select_reply_lands_as_runnable_sql`.

- **Error paths retained a stale result grid (low→fixed).** Neither error path cleared `self.result`: `set_query_error` (preprocess reject) and the `QueryResponse::Error` arm updated only status + phase, so a previously-successful grid (and its truncation banner) stayed painted under the error message — contradicting both `set_query_error`'s own comment and the `empty_state` doc ("a query that errored leaves no grid"). **Chose the contract and made code + comments + tests agree:** a new `App::clear_result` (drops `result`, resets `result_ciq_capped`/`v_row_offset`/`h_col_offset`) is called on both error paths and the empty-bar branch, so the pane falls back to the neutral empty-state. Guards: `polish_tests::{engine_error_after_a_success_clears_the_stale_grid_and_banner, preprocess_reject_after_a_success_clears_the_stale_grid}`.

- **did-you-mean gave a vacuous suggestion for an empty / 1-char unknown column (low→fixed).** `nearest_column` treated `is_subsequence(name, typo)` as a close match, which is vacuously true for an empty needle and trivially true for a 1-char needle — so `""`/`a`/`r` resolved to the first/shortest column ("did you mean…?") rather than the bare message. **Guarded degenerate typos:** `nearest_column` returns `None` when the trimmed typo is under two chars (below which there is no signal — better no hint than a misleading one). Guards: `error_enhance_tests::{did_you_mean_omitted_for_a_single_char_token, did_you_mean_omitted_for_an_empty_token}`.

- **`src/app.rs` over the 1000-line limit (low→fixed).** Phase 5 grew `app.rs` past the hard 1000-line cap. **Lifted the autocomplete orchestration** (`refresh_autocomplete`, `value_column_to_fetch`, the free `distinct_values`) into a sibling `impl App` file `src/app/autocomplete_app.rs` (the same pattern as `ai_app`/`history_app`/`polish`), removing six now-unused imports from `app.rs` and bringing it back under the cap (992 lines).

- **`output-modes.md` documented a non-existent interactive OSC52 clipboard feature pointing at an empty key set (medium→fixed, docs).** `clipboard::osc52::copy` is never invoked from any non-test path and no copy chord exists in any App handler, yet the page described an in-session OSC 52 copy path and told readers to "See the Quick Reference for the in-session key set" (which has none — a dangling cross-reference, a P5.4 exit-criterion miss). **Rewrote the section** to the truthful capability: there is no in-session copy chord; pipe the `--output` result to your OS clipboard tool (`pbcopy`/`xclip`/`clip`). The broken Quick-Reference pointer is removed.
